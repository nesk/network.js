...
export default class BandwidthModule extends HttpModule {
    ...
    settings(settings = null) {
        ...
        // If the endpoint has a getter, alter it to provide the current request size.
        const endpointDescriptor = Object.getOwnPropertyDescriptor(this._settings, 'endpoint');
        if (isFunction(endpointDescriptor.get)) {
            // We can't use the `settings` method to retrieve the data size or this will trigger the `endpoint` getter,
            // thus triggering an infinite loop. Use the internal property instead.
            endpointDescriptor.get = () => endpointDescriptor.get(this._settings.data.size);

            Object.defineProperty(this._settings, 'endpoint', endpointDescriptor);
        }
    }
